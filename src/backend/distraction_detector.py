...
face_cascade = cv2.CascadeClassifier('src/backend/haarcascade_files/haarcascade_frontalface_default.xml')
eye_cascade = cv2.CascadeClassifier('src/backend/haarcascade_files/haarcascade_eye.xml')
distract_model = load_model('src/backend/cnn/distraction_model.hdf5', compile=False)
    ...
    if ret:
        ...
        if len(faces) > 0:
            ...
            for (x,y,w,h) in faces:
                ...
                url = 'http://localhost:3000/api/distraction/get_distraction'
                data = {'distractionStatus': label}
                ...
                if response.status_code == 200:
                    print('Distraction status sent successfully.')
                    print('Response from server:', response.json())
                else:
                    print('Failed to send distraction status. Server returned status code:', response.status_code)

                

                 # Send the distraction status to the server
                #url = 'http://localhost:3000/api/distraction/get_distraction/' + label
                ...
                #    print('Distraction status sent successfully')
                #else:
                #    print('Failed to send distraction status')
